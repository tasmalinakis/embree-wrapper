...
namespace intersector
{
	...
	int isectCreateIntersector(size_t ray_bunch_size_hint)
	{
		return _createIntersector(ray_bunch_size_hint);
	}
	

	isect_error_t isectDestroyIntersector(int iid)
	{
		return _destroyIntersector(iid);
	}
	...
	isect_error_t isectAddTriangles(int iid, float *vertex_data, void **prim_ptr,
		float *world_transform, size_t num_triangles)
	{
		return _addTriangles(iid, vertex_data, prim_ptr, world_transform, num_triangles);
	}
	
	// Clear all triangles in the iid intersector
	isect_error_t isectResetTriangles(int iid)
	{
		return _resetTriangles(iid);
	}
	...
	isect_error_t isectTraceRays(int iid, float *origin_data, float *point_at_data,
		float * intersection_data, void ** hit_primitive_ptr,
		size_t num_rays, isect_ray_type_t r_type, float max_range)
	{
		return _traceRays(iid, origin_data, point_at_data, intersection_data, hit_primitive_ptr, num_rays, r_type, max_range);
	}
